...
"""
    ...
    It is needed to generate correct API access tokens for user.
    
    If permissions/scopes of the Development Keys change, this needs to be called again to update tokens.
"""
...
"""
    When creating a listing\inventory item, the fulfillment, payment, and return policies are required in the body.
    
    I am assuming that the user has already created these on their seller account.
"""
...
"""
    This function needs to be called whenever the access token is no longer valid. Generally, access tokens last for 2 hours.
 
    Access token validity can be checked by calling check_access_token_validity(), or checking response of any allowed API call.
    
    If this fails, it means the refresh token is invalid, and tokens must all be regenerated by calling generate_tokens_initial().
"""   
...
"""
    Every listing with SKU needs a corresponding inventory item with the same SKU.
    
    This is called inside of create_test_listing(), so that only a single function needs to called in order to create a listing.
""" 
...
"""
    ...
    Byproduct being it also deletes all listings as well, due to inventory\listing relationship.
"""
...
"""
    ...
    This function will fail if any of create_test_inventory_item(), offer POST, or publish POST fail.  
"""    
...
"""
    This function handles cases where inventory/SKU already exists when creating a listing. 
    
    This should not happen frequently, as create_test_listing() handles inventory creation as well.
"""
...
"""
    This function is need in order to create a listing. Inventory creation requires at least one image to be attatched to body.
    
    I could not find a way to do this through REST API, so I had to use the XML TRADING instead.
    
    This is slowest part of listing creation process.
"""
...
"""
    Any API call could check if access token works, but I thought it would be convinient to have a simple function to 
    have a quick request on standby when needed.
"""
